...
def train_model():
    ...
    data_from_es = Preprocessing.get_log_from_es(esclient=es, search_size=95000)
    ...
    if RESUME:
        print("从节点恢复训练")
        path_checkpoint = r"D:\zpg\paper2\code\UEBA_GAN\gan4netflow\implementations\wgan_gp\checkpoint\230309checkpoint_20.pth"  # 断点路径
        checkpoint = torch.load(path_checkpoint)  # 加载断点
        print("==============恢复网络参数===============")
        discriminator.load_state_dict(checkpoint['discriminator_net'])  # 加载模型可学习参数
    ...
    for epoch in range(start_epoch + 1, opt.n_epochs):
        ...
        if epoch % 20 == 0:
            ...
            print("保存当前训练节点")
            checkpoint = {
                "discriminator_net": discriminator.state_dict(),
                "generator_net": generator.state_dict(),
                'optimizer_D': optimizer_D.state_dict(),
                'optimizer_G': optimizer_G.state_dict(),
                "epoch": epoch
            }
            if not os.path.isdir(opt.save_checkpoint_dir):
                os.mkdir(opt.save_checkpoint_dir)
            # torch.save(checkpoint, r'D:\zpg\paper2\code\UEBA_GAN\gan4netflow\implementations\wgan_gp\checkpoint\checkpoint\ckpt_best_%s.pth' % (str(epoch)))
            torch.save(checkpoint,
                       f'%s\\{savetime}checkpoint_%d.pth' % (opt.save_checkpoint_dir, epoch))


        batch_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
        print(batch_time)

def detectionbymodel():
    # ----------
    #  载入模型及参数
    # ----------
    ...
    try:
        test_Preprocessing = Preprocessing()  # 载入数据预处理类
        es = Elasticsearch("http://172.23.6.213:9200")  # 创建es客户端
        data_from_es = Preprocessing.get_log_from_es(esclient=es, search_size=1)
    ...
    #  检测
    # ----------
    predict_samples = Variable(predict_flow.type(Tensor)) if cuda else Variable(
        predict_flow.float())
